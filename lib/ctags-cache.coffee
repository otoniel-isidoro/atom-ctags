...
module.exports =
  ...
  readTags: (p, container, callback) ->
    console.log "[atom-ruby-ctags:readTags] #{p} start..."
    startTime = Date.now()

    stream = ctags.createReadStream(p)

    stream.on 'error', (error)->
      console.error 'atom-ruby-ctags: ', error
    ...
    stream.on 'data', (tags)->
      for tag in tags
        ...
        data.push tag
    stream.on 'end', ()->
      console.log "[atom-ruby-ctags:readTags] #{p} cost: #{Date.now() - startTime}ms"
      callback?()
  ...
  findTags: (prefix, options) ->
    ...
    console.warn("[atom-ruby-ctags:findTags] tags empty, did you RebuildTags or set extraTagFiles?") if tags.length == 0
    return tags
  ...
  generateTags:(p, isAppend, callback) ->
    ...
    startTime = Date.now()
    console.log "[atom-ruby-ctags:rebuild] start @#{p}@ tags..."

    cmdArgs = atom.config.get("atom-ruby-ctags.cmdArgs")
    cmdArgs = cmdArgs.split(" ") if cmdArgs

    TagGenerator p, isAppend, @cmdArgs || cmdArgs, (tagpath) =>
      console.log "[atom-ruby-ctags:rebuild] command done @#{p}@ tags. cost: #{Date.now() - startTime}ms"
